...
for i in range(len(ticker_list[0])):
    ...
    if ticker_list[0][i] in delisting_list:
        a = np.zeros((data_len,1))
    else:
...
add_mat = np.concatenate([index_mat, all_mat], axis=0)
#all_mat = np.concatenate([index_mat[:,0:6], all_mat], axis=0)
add_mat = np.delete(all_mat, 1, 0)

add_filename = "../data/temp_add_stock_data.csv"
np.savetxt(add_filename, all_mat, delimiter=',', fmt='%s')

"""
list = []
filename = "../data/stock_daily_data.csv"
list.append(pd.read_csv(filename))
list.append(pd.read_csv(add_filename))
