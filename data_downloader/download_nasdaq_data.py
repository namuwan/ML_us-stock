...
index_mat = np.array(["date","NASDAQ","","IXIC"])
index_mat = np.reshape(index_mat, (2,2))

start_date = "2003-1-1"
end_date = "2020-9-21"

nasdaq_mat = tiingo_download(".IXIC", start_date, end_date)
nasdaq_mat = np.concatenate([index_mat, nasdaq_mat], axis=0)

print(sp500_mat)
np.savetxt("../data/nasdaq_daily_data.csv", sp500_mat, delimiter=',', fmt='%s')
