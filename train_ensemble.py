...
model_gbdt_cv1 = GradientBoostingRegressor(**params_gbdt)
model_gbdt_cv2 = GradientBoostingRegressor(**params_gbdt)
model_gbdt_cv4 = GradientBoostingRegressor(**params_gbdt)
model_gbdt_all = GradientBoostingRegressor(**params_gbdt)
print("GBDT trained")
#ExtraTrees
params_et = {
...
model_et_cv1 = ExtraTreesRegressor(**params_et)
model_et_cv2 = ExtraTreesRegressor(**params_et)
model_et_cv4 = ExtraTreesRegressor(**params_et)
model_et_all = ExtraTreesRegressor(**params_et)
...
model_xgb_cv1 = xgb.XGBRegressor(**params_xgb)
model_xgb_cv2 = xgb.XGBRegressor(**params_xgb)
model_xgb_cv4 = xgb.XGBRegressor(**params_xgb)
model_xgb_all = xgb.XGBRegressor(**params_xgb)
print("XGBoost trained")

print()

model_gbdt_cv1.fit(X_CV1, Y_CV1)
model_gbdt_cv2.fit(X_CV2, Y_CV2)
model_gbdt_cv4.fit(X_CV4, Y_CV4)
model_gbdt_all.fit(X_all, Y_all)
print("GBDT fitted")
model_et_cv1.fit(X_CV1, Y_CV1)
model_et_cv2.fit(X_CV2, Y_CV2)
model_et_cv4.fit(X_CV4, Y_CV4)
model_et_all.fit(X_all, Y_all)
print("ExtraTrees fitted")
model_xgb_cv1.fit(X_CV1, Y_CV1)
model_xgb_cv2.fit(X_CV2, Y_CV2)
model_xgb_cv4.fit(X_CV4, Y_CV4)
model_xgb_all.fit(X_all, Y_all)
print("XGBoost fitted")

"""
#CV2予想で 各手法(GBDT,ET,XGB)の相関を見る。結果:GBDT,ETは似ているが、XGBは前記2手法と異なる)
X_def_deleted = mfiv.delete_deficit_col(X[split_ts_pos1*stock_num:split_ts_pos2*stock_num,:], -999)
y_gbdt_predicted_1_2 = model_gbdt_cv1.predict(X_def_deleted[:,1:])
y_et_predicted_1_2 = model_et_cv1.predict(X_def_deleted[:,1:])
y_xgb_predicted_1_2 = model_xgb_cv1.predict(X_def_deleted[:,1:])
coef_1_2_gbdt_et = np.corrcoef(y_gbdt_predicted_1_2, y_et_predicted_1_2)[0,1]
print("coef_cv1-2_GBDT vs ET", end=" ")
print(coef_1_2_gbdt_et)
coef_1_2_gbdt_xgb = np.corrcoef(y_gbdt_predicted_1_2, y_xgb_predicted_1_2)[0,1]
print("coef_cv1-2_GBDT vs XGB", end=" ")
print(coef_1_2_gbdt_xgb)
coef_1_2_et_xgb = np.corrcoef(y_et_predicted_1_2, y_xgb_predicted_1_2)[0,1]
print("coef_cv1-2_ET vs XGB", end=" ")
print(coef_1_2_et_xgb)
"""
"""
X_def_deleted = mfiv.delete_deficit_col(X[split_ts_pos2*stock_num:timestamp_num*stock_num,:], -999)
y_gbdt_predicted_1_3 = model_gbdt_cv1.predict(X_def_deleted[:,1:])
y_et_predicted_1_3 = model_et_cv1.predict(X_def_deleted[:,1:])
y_xgb_predicted_1_3 = model_xgb_cv1.predict(X_def_deleted[:,1:])
coef_1_3_gbdt_et = np.corrcoef(y_gbdt_predicted_1_3, y_et_predicted_1_3)[0,1]
print("coef_cv1-3_GBDT vs ET", end=" ")
print(coef_1_3_gbdt_et)
coef_1_3_gbdt_xgb = np.corrcoef(y_gbdt_predicted_1_3, y_xgb_predicted_1_3)[0,1]
print("coef_cv1-3_GBDT vs XGB", end=" ")
print(coef_1_3_gbdt_xgb)
coef_1_3_et_xgb = np.corrcoef(y_et_predicted_1_3, y_xgb_predicted_1_3)[0,1]
print("coef_cv1-3_ET vs XGB", end=" ")
print(coef_1_3_et_xgb)
"""
...
for i in range(split_ts_pos1,split_ts_pos2):
    ...
    if X_def_deleted.shape[0]!=0: #deficitで行数0になる可能性がある
        #y_gbdt_predicted = model_gbdt_cv1.predict(X_def_deleted[:,1:])
        y_et_predicted = model_et_cv1.predict(X_def_deleted[:,1:])
        y_xgb_predicted = model_xgb_cv1.predict(X_def_deleted[:,1:])
        y_predicted = 0.5*y_et_predicted + 0.5*y_xgb_predicted
...
for i in range(split_ts_pos2,timestamp_num):
    X_def_deleted = mfiv.delete_deficit_col(X[i*stock_num:(i+1)*stock_num,:], -999)
    if X_def_deleted.shape[0]!=0: #deficitで行数0になる可能性がある
        #y_gbdt_predicted = model_gbdt_cv2.predict(X_def_deleted[:,1:])
        y_et_predicted = model_et_cv2.predict(X_def_deleted[:,1:])
        y_xgb_predicted = model_xgb_cv2.predict(X_def_deleted[:,1:])
        y_predicted = 0.5*y_et_predicted + 0.5*y_xgb_predicted
        y_result = X_def_deleted[:,0]
        coef = np.corrcoef(y_predicted, y_result)[0,1]
...
for i in range(split_ts_pos3, timestamp_num):
    X_def_deleted = mfiv.delete_deficit_col(X[i*stock_num:(i+1)*stock_num,:], -999)
    if X_def_deleted.shape[0]!=0: #deficitで行数0になる可能性がある
        #y_gbdt_predicted = model_gbdt_cv4.predict(X_def_deleted[:,1:])
        y_et_predicted = model_et_cv4.predict(X_def_deleted[:,1:])
        y_xgb_predicted = model_xgb_cv4.predict(X_def_deleted[:,1:])
        y_predicted = 0.5*y_et_predicted + 0.5*y_xgb_predicted
        y_result = X_def_deleted[:,0]
        coef = np.corrcoef(y_predicted, y_result)[0,1]
...
for i in range(0, timestamp_num):
    X_def_deleted = mfiv.delete_deficit_col(X[i*stock_num:(i+1)*stock_num,:], -999)
    if X_def_deleted.shape[0]!=0: #deficitで行数0になる可能性がある
        #y_gbdt_predicted = model_gbdt_all.predict(X_def_deleted[:,1:])
        y_et_predicted = model_et_all.predict(X_def_deleted[:,1:])
        y_xgb_predicted = model_xgb_all.predict(X_def_deleted[:,1:])
        y_predicted = 0.5*y_et_predicted + 0.5*y_xgb_predicted
        y_result = X_def_deleted[:,0]
        coef = np.corrcoef(y_predicted, y_result)[0,1]
